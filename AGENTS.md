...
- Before committing, run `python -m py_compile skills/animated-sticker-maker/scripts/*.py`, `python -m unittest discover -s tests -v`, `ruff check .`, and `git diff --check HEAD`. Generate a report-only coverage overview with `coverage run -m unittest discover -s tests && coverage report -m`.
